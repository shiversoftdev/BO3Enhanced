...
HMEMORYMODULE MemoryLoadLibrary(const void *data, size_t size, void* modBase, BOOL noCallbacks)
{
    return MemoryLoadLibraryEx(data, size, MemoryDefaultAlloc, MemoryDefaultFree, MemoryDefaultLoadLibrary, MemoryDefaultGetProcAddress, MemoryDefaultFreeLibrary, NULL, modBase, noCallbacks);
}
...
HMEMORYMODULE MemoryLoadLibraryEx(const void *data, size_t size,
    ...
    CustomGetProcAddressFunc getProcAddress,
    CustomFreeLibraryFunc freeLibrary,
    void *userdata, void* modBase, BOOL noCallbacks)
{
    ...
    code = (unsigned char *)allocMemory(modBase ? modBase : (LPVOID)(old_header->OptionalHeader.ImageBase),
        alignedImageSize,
    ...
    if (!noCallbacks && !ExecuteTLS(result)) {
        goto error;
    }

    // get entry point of loaded library
    if (!noCallbacks && (result->headers->OptionalHeader.AddressOfEntryPoint != 0)) {
        if (result->isDLL) {
